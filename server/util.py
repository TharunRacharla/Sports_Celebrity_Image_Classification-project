...
def load_saved_artifacts():
    ...
    with open("E:\\Programming\\Data Science\\imagerecognition\\server\\artifacts\\class_dictionary.json", "r") as f:
        __class_name_to_number = json.load(f)
        __class_number_to_name = {v:k for k,v in __class_name_to_number.items()}

    global __model
    if __model is None:
        with open('E:\\Programming\\Data Science\\imagerecognition\\server\\artifacts\\saved_model.pkl', 'rb') as f:
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    face_cascade = cv2.CascadeClassifier('E:\Programming\Data Science\imagerecognition\model\opencv\haarcascades\haarcascade_frontalface_default.xml')
    eye_cascade = cv2.CascadeClassifier('E:\Programming\Data Science\imagerecognition\model\opencv\haarcascades\haarcascade_eye.xml')
